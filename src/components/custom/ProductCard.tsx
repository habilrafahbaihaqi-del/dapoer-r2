...
export function ProductCard({ product, onSelect }: ProductCardProps) {
  ...
  return (
    <Card className="overflow-hidden flex flex-col justify-between h-full group hover:shadow-lg transition-all duration-300">
      <div>
        {/* Kontainer Gambar 16:9 (aspect-video) */}
        <div className="relative w-full aspect-video bg-gray-100 overflow-hidden">
          {/* Komponen Image Next.js untuk optimasi gambar */}
          <Image
            src={product.image}
            alt={product.name}
            fill
      ...
      <CardFooter className="p-5 pt-0 flex flex-col items-start gap-4">
        ...
        <Button
          className="w-full bg-[#651114] hover:bg-[#4a0d0f] text-white font-semibold tracking-wide transition-all duration-300 hover:shadow-md"
